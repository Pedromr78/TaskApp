...
const onsubmit= () => {
  ...
  const requestOptionstocken = {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(formtocken)
  };
  ...
  .then(async response => {
    ...
    if(usersave.status == 'success'){
          localStorage.setItem("user", JSON.stringify(usersave.user))
    ...
    fetch("http://localhost:3000/users/login", requestOptionstocken)
    .then(async response => {
     
        const tockensave= await response.json()
      
        if(tockensave.status== 'success'){
          tocken.value= tockensave
          localStorage.setItem("tocken", JSON.stringify(tockensave.tocken))
